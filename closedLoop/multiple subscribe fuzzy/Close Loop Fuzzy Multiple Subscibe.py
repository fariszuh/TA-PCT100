...
de_np = np.array([0,0])
# MQTT------------------
def on_message_SP_LEVEL(client, userdata, message):
    global SP_LEVEL
...
def on_message_SP_cs(client, userdata, message):
    global SP_cs
    cs = str(message.payload.decode("utf-8"))
    # print("received message: " ,str(message.payload.decode("utf-8")))
    # return SP
    print("SP: " + str(cs))

def on_message_SP_cf(client, userdata, message):
    global SP_cf
    cf = str(message.payload.decode("utf-8"))
    # print("received message: " ,str(message.payload.decode("utf-8")))
    # return SP
    print("SP: " + str(cf))
    
def on_message_SP_os(client, userdata, message):
    global SP_os
    os = str(message.payload.decode("utf-8"))
    # print("received message: " ,str(message.payload.decode("utf-8")))
    # return SP
    print("SP: " + str(os))

def on_message_SP_of(client, userdata, message):
    global SP_of
    of = str(message.payload.decode("utf-8"))
    # print("received message: " ,str(message.payload.decode("utf-8")))
    # return SP
    print("SP: " + str(of))
...
def kontroller():
    ...
    e_l = trimf(efuzzy, [-4, 1 , 6])[n]  # error low
    e_ok = trimf(efuzzy, [1 , 6 , 11])[n]  # error okey
    e_h = trimf(efuzzy, [6 , 11 , 16])[n]  # error high
    if efuzzy[n] > 1 :
        e_h = 1
    elif efuzzy[n] < -1 :
        e_l = 1
