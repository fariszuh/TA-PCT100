...
#define num_neuronHidden 3 // sebagai baris
#define num_neuronInput	 3 // sebagai kolom
...
float y, Qin; // dalam volt
float SP = 100; // dalam mm
uint8_t dey_deu;
...
float AO1_volt = 4; // pompa, drain valve
...
// Initialize learning rate, alpha, delta1, and control signal u
float lr = 0.00001;
float alpha = 1;
float delta1 = 0;
float u = 0;
...
int awal = 1;
char stringKirim[] = "k, timestamp, MV_volt_FCValve, MV_volt_drain, volt_flow, volt_level, volt_PA2, lr, alpha, SP, e_0 \n";
int k = 0;
uint32_t timestamp;
...
//	while(1);
 ...
 /* Check free space */
  ...
  if(SP < 42.1779848866499){
	  SP = 42.1779848866499;
  }
  else if(SP > 217.324332493703){
	  SP = 217.324332493703;
  }
  // mm to volt conversion
  SP = SP*0.057081170862780 - 2.407568761962603;
  ...
  while (1)
	...
	AI0_volt = (float) AI0_bit * 10 / 4096; // PA0
	AI1_volt = (float) AI1_bit * 10 / 4096; // PA1
	AI2_volt = (float) AI2_bit * 10 / 4096; // PA2
	sprintf(stringKirim,"%d, %d, %f, %f, %f, %f, %f, %f, %f, %f, %f \n",k,timestamp, AO0_volt, AO1_volt, AI0_volt, AI1_volt, AI2_volt, lr, alpha, SP, e_0); // stringKirim baru, sesuai nilai adc cpltCallback
...
//	}
	...
	if(diffTime > 3000){ // awal kali 0V, setelah 2s keluar AO sesuai yg diminta
				// Singularity function: variasi beban Ao mov
				if(diffTime < 123000){
					AO1_volt = 4;
				}
				if(diffTime > 123000 && diffTime < 243000){
					AO1_volt = 7.5;
				}
				if(diffTime > 243000 && diffTime < 363000){
					AO1_volt = 10;
				}
				if(diffTime > 363000 && diffTime < 483000){
					AO1_volt = 7.5;
				}
				if(diffTime > 483000){
					AO1_volt = 4;
...
//				AO1_volt = 10; // pompa, drain valve
			    ...
			    u = 1/(1 + exp(-1*r));
			    AO0_volt = 10*u + 12; // valve flow control Qin, multiplied by gain 22Vfc

			    // CALCULATE ERROR
			    Qin = (float) AI0_bit*10*6.940753740463935*0.06 / 4096;
			    y = (float) AI1_bit*10 / 4096; // y dalam volt
			    e_0 = SP - y;
...
//			    }
				...
				if(diffTime > 603000) { // waktu stop untuk close dan unmount csv, nilai ini harus + difftime 2s
					f_close(&fil); //Close file, don't forget this!
